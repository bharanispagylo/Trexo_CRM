    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const [userData, taskData, worklogData] = await Promise.all([
          ...
          fetchRoute('GET /worklogs', '/worklogs')
        ]);
        setUsers(userData || []);

        const m0 = performance.now();
        const filteredTasks = (taskData || []).filter(t => t.status !== 'Archived' && t.status !== 'Archive' && (t.taskType || '').toLowerCase() !== 'calls/meetings');
        const dProc = Number((performance.now() - m0).toFixed(2));
        methodTimings.push({ name: 'Active Task Filter Processing', duration: dProc });
        ...
        logPagePerf('DAILY LOAD ALL PAGE', apiTimings, methodTimings, pageStartMs);
      } catch (err) {
