...
export default function Projects({ user, initialSelectedProject, onClearInitialProject, onNavigateToTasks, initialProjectName, onProjectSelect }) {
  ...
  const fetchData = async (silent = false) => {
    ...
    try {
      ...
      const dUpd = Number((performance.now() - m0).toFixed(2));
      methodTimings.push({ name: 'Selected Project Sync Processing', duration: dUpd });
      ...
      if (!silent) {
        ...
        logPagePerf('PROJECTS PAGE', apiTimings, methodTimings, pageStartMs);
      }
    } catch (error) {
          ...
          {detailTab === 'Tasks' && (
            <div>
              ...
              {(selectedProject.taskLists || []).length === 0 ? (
                ...
                <div className="cu-list-root" style={{ display: 'flex', flexDirection: 'column', gap: '-0.25rem', border: 'none', background: 'transparent', boxShadow: 'none' }}>
                  {(() => {
